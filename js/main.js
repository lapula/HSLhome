...
function getStopDataFromLocation(position) {
    if (!tizen.preference.exists('stops')) {return;}
	var stops = tizen.preference.getValue('stops').split(",");
	
    Promise.all([queryStopData(stops), 
                 queryStopsByCoordinates(position.coords.latitude,
...
function getStopDataWithoutLocation() {
	if (!tizen.preference.exists('stops')) {return;}
	var stops = tizen.preference.getValue('stops').split(",");
	
	queryStopData(stops).then(function(response){
...
window.onload = function () {
	...
	if (!tizen.preference.exists('stops')) {
		tizen.preference.setValue('stops', "0412,0401");
	}
